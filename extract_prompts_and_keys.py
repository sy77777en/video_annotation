...
This generates task_info_summary.json which is used by caption_to_json_api.py.

IMPORTANT: 
- This script extracts JSON keys FROM the actual template being used
- Custom templates may have different JSON structures than json_policy.json
- The keys extracted here are what the LLM is being asked to produce
"""
...
def get_json_policy_for_task(json_policy: Dict[str, Any], task: str) -> Dict[str, Any]:
    """Get the JSON policy structure for a specific task from json_policy.json"""
...
def extract_json_from_template(template: str) -> Tuple[Optional[Dict[str, Any]], Optional[str]]:
    """
    Extract the JSON structure embedded in a template.
    
    Returns:
        (parsed_json, json_string) or (None, None) if not found
    """
    # Find the first { that looks like start of JSON
    start_idx = -1
    for i, char in enumerate(template):
        if char == '{':
            # Check if this looks like JSON start (has a key after it)
            rest = template[i:i+100]
            if '"' in rest:
                start_idx = i
                break
...
def get_code_generated_template(task: str, json_policy: Dict[str, Any], caption_instruction: str) -> str:
    ...
    THIS IS EXACTLY THE SAME AS test_llm.py's get_prompt_template_for_task()
    """
    short_name = TASK_FULL_TO_SHORT.get(task, task)
    ...
    # Base template for camera/subject/motion tasks
...
6. Return only valid JSON without any additional text"""
    ...
    # Task-specific additions (EXACTLY matching test_llm.py)
...
8. Mention only subject action in "subject_action". Avoid mentioning camera related details"""
        ...
        # NOTE: The else branch in test_llm.py has DIFFERENT instruction #2
        # It's missing "with all the keywords and details"
...
def get_default_template(root_path: Path, task: str, json_policy: Dict[str, Any], caption_instruction: str) -> Tuple[str, str]:
    """
    Get the default template for a task.
    
    Returns:
        (template_content, source) where source is "custom:{name}" or "code_generated"
    """
    templates_dir = get_templates_dir(root_path, task)
    default_ref_file = templates_dir / "default.txt"
    
    # Check for custom default reference (same logic as test_llm.py load_template)
    if default_ref_file.exists():
        ...
        if template_path.exists():
            with open(template_path, 'r') as f:
                return f.read(), f"custom:{ref_name}"
    
    # Fall back to code-generated default
    return get_code_generated_template(task, json_policy, caption_instruction), "code_generated"
...
def main():
    ...
    # Load JSON policy (used for code-generated templates)
    ...
    for task_full, task_short in TASK_FULL_TO_SHORT.items():
        ...
        # Get JSON policy for this task (from json_policy.json)
        ...
        # Get default template (FULL template content)
        default_template, template_source = get_default_template(
            root_path, task_full, task_json_policy, caption_instruction
        )
        
        # Parse template source
        using_custom = template_source.startswith("custom:")
        custom_name = template_source.split(":")[1] if using_custom else None
        
        # ============================================================
        # CRITICAL: Extract JSON keys FROM the actual template
        # This handles custom templates that may have different keys
        # ============================================================
        extracted_json, extracted_json_str = extract_json_from_template(default_template)
        
        if extracted_json:
            json_keys = extract_top_level_keys(extracted_json)
            json_structure = extracted_json
            print(f"\n✅ Extracted JSON keys from template: {json_keys}")
        else:
            # Fallback to json_policy.json keys
            json_keys = extract_top_level_keys(task_json_policy)
            json_structure = task_json_policy
            print(f"\n⚠️ Could not extract JSON from template, using json_policy.json keys: {json_keys}")
        
        # Compare with json_policy.json keys if different source
        if using_custom:
            policy_keys = extract_top_level_keys(task_json_policy)
            if set(json_keys) != set(policy_keys):
                print(f"   📋 Note: Custom template keys differ from json_policy.json")
                print(f"      Template keys: {json_keys}")
                print(f"      Policy keys:   {policy_keys}")
        
        print(f"\n📄 Template Source: {template_source}")
        print(f"\n📄 Caption Instruction (first 200 chars):")
        print("   " + caption_instruction[:200].replace('\n', '\n   ') + "...")
        print(f"\n📄 Default Template (first 500 chars):")
        print("   " + default_template[:500].replace('\n', '\n   ') + "...")
        
        # Store ALL info including full template
        all_task_info[task_short] = {
            "task_full": task_full,
            "task_short": task_short,
            "json_keys": json_keys,  # Keys extracted FROM the template
            "json_structure": json_structure,  # Full JSON structure from template
            "json_policy": task_json_policy,  # Original from json_policy.json (for reference)
    ...
    # Save COMPLETE info to JSON file
    summary_path = root_path / "json_policy" / "task_info_summary.json"
    
    with open(summary_path, 'w') as f:
        json.dump(all_task_info, f, indent=2)
    ...
    for task_short, info in all_task_info.items():
        keys = info["json_keys"]
        source = info["template_source"]
        print(f"\n{task_short}:")
        print(f"  Keys (from template): {keys}")
